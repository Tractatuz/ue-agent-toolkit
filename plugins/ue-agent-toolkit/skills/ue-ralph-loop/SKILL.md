...
Useful options include `--goal-file`, `--input-packet`, `--spec`, `--plan`, `--max-iterations`, `--model`, `--sandbox`, `--codex-bin`, `--timeout-seconds`, `--artifacts-dir`, and `--dry-run`.

- Run attempts sequentially because they share a working tree and Unreal Editor.
- Default to `workspace-write`; use broader access only with explicit user authorization and appropriate isolation.
- A new Codex task must load the ue-agent-toolkit plugin's Unreal MCP connection and start by rediscovering live toolsets.
...
- Follow `AGENTS.md`; preserve user changes and never revert without permission.
