...
The preferred model is a step-based loop attempt. Each attempt follows the same ordered steps: re-check spec, revise plan, analyze project context, implement, validate/test, evaluate/report, then decide whether to launch the next attempt in a fresh `codex exec` session.
...
If Step 7 is not complete and another attempt is justified, write a self-contained re-entry packet and start the next attempt in a fresh `codex exec --ephemeral` session. The new attempt must begin again at Step 1, then re-check spec and revise the plan before implementation.
...
Run this command with the working directory set to the directory containing this `SKILL.md`, unless an absolute script path is used.
...
- `--model <model>`: pass a model override to fresh `codex exec` attempt sessions.
- `--sandbox <mode>`: choose `read-only`, `workspace-write`, or `danger-full-access`; default is `workspace-write`.
- `--codex-bin <path>`: override the `codex` executable name or path.
...
Use `danger-full-access` only when the user explicitly accepts the risk and the environment is appropriately isolated.
...
- Keep `ue-code-writer` custom-agent work parallel only when file scopes are independent.
...
Use the installed Unreal custom agents through their owning workflow skills rather than bypassing the specialized workflows.
...
- Use `ue-code-reviewer` through `ue-implement` for independent review of non-trivial C++ or config changes.
...
- After adding, updating, or reinstalling the plugin, tell the user to start a new Codex task before relying on the changed skill definitions.
