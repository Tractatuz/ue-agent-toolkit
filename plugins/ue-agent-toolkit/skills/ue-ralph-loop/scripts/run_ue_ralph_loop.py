#!/usr/bin/env python3
"""Run Ralph Loop attempts in fresh Claude Code sessions.

This launcher starts one fresh `claude --print` attempt session per iteration.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an Unreal Ralph Loop through fresh Claude Code attempt sessions."
    )
    ...
    parser.add_argument("--project-root", default=".", help="Project root used as the attempt working directory.")
    parser.add_argument(
        "--artifacts-dir",
    ...
    parser.add_argument("--claude-bin", default="claude", help="Claude Code executable name or path.")
    parser.add_argument("--model", help="Optional model passed to claude --model.")
    parser.add_argument(
        "--permission-mode",
        choices=("default", "plan", "acceptEdits", "bypassPermissions"),
        default="acceptEdits",
        help="Permission mode for each fresh Claude Code attempt.",
    )
    parser.add_argument("--timeout-seconds", type=int, help="Timeout per fresh Claude Code attempt.")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Write the initial packet and print the first Claude Code command without running it.",
    )
...
- Do not spawn another Claude Code session during this attempt.
...
def build_attempt_prompt(
    ...
    return f"""Use the `ue-agent-toolkit:ue-ralph-loop` skill and follow its ordered loop steps.

This is iteration {iteration} for feature `{feature}`. This Claude Code run is the fresh session for one bounded loop attempt.
...
- Step 8: do not start another Claude Code session directly; instead write status JSON indicating whether another fresh attempt is needed.
...
def build_command(
    ...
    # ponytail: shutil.which resolves the Windows `claude.cmd` shim that bare CreateProcess misses.
    command = [
        shutil.which(args.claude_bin) or args.claude_bin,
        "--print",
        "--permission-mode",
...
def main() -> int:
    ...
    for iteration in range(1, args.max_iterations + 1):
        ...
        print(f"\nIteration {iteration}: starting fresh Claude Code session")
        print("Command: " + " ".join(json.dumps(part) for part in command[:-1]) + " <prompt>")

        if args.dry_run:
            print("Dry run enabled; stopping before the first claude run.")
            return 0

        try:
            completed = run_child(command, project_root, args.timeout_seconds)
            ...
            write_json(
                status_json,
                {
                    "status": "blocked",
                    "continue": False,
                    "summary": "Fresh Claude Code attempt timed out.",
        ...
        if completed.returncode != 0:
            write_json(
                status_json,
                {
                    "status": "blocked",
                    "continue": False,
                    "summary": "Fresh Claude Code attempt exited with a non-zero status.",
                    "next_starting_gate": "Blocked",
                    "first_actionable_failure": f"claude exit code {completed.returncode}. See stderr log.",
                    "result_packet": str(result_packet),
                },
            )
